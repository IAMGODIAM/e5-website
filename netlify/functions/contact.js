...
// Env vars: M365_CLIENT_ID, M365_CLIENT_SECRET, M365_TENANT_ID
...
Submitted via e5enclave.com`;
    ...
    // Try M365 Graph API notification
    let sent = false;
    if (process.env.M365_CLIENT_ID && process.env.M365_CLIENT_SECRET && process.env.M365_TENANT_ID) {
      try {
        ...
        sent = true;
        console.log(`[contact] Sent via M365: ${email} / ${subjectLine}`);
      } catch (graphErr) {
        console.error('[contact] Graph API error:', graphErr.message);
      }
    }

    // Always log to Netlify (backup — logs retained 30 days)
    console.log(JSON.stringify({
      type: 'form_submission',
      email, name, subject: subjectLine, organization, track, pillar, role, source,
      sent_via_m365: sent,
    ...
    return { statusCode: 500, headers, body: JSON.stringify({ error: 'Submission failed. Please email accounts@e5enclave.com directly.' }) };
  }
};
